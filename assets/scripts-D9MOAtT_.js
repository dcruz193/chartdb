import{D as t}from"./database-type-C2NNW5SW.js";import{d as S,b as a,c as A}from"./code-editor-CVERTBtE.js";import"./index-D22tuFcn.js";import"./canvas-utils-C5DtV3-P.js";import"./theme-provider-qAUrzat1.js";import"./databases-C9KYyYJn.js";import"./use-storage-CGFXC_qQ.js";import"./label-DEegoE4u.js";import"./storage-provider-DM7k1KMT.js";import"./env-B5QThsD5.js";const N=(n={})=>{const e=n.databaseEdition,E=`
...
FROM fk_info${e?"_"+e:""}, pk_info, cols, indexes_metadata, tbls, config, views;
    `,r=`# *** Remember to change! (HOST_NAME, PORT, USER_NAME, DATABASE_NAME) *** 
`;return n.databaseClient===A.POSTGRESQL_PSQL?`${r}psql -h HOST_NAME -p PORT -U USER_NAME -d DATABASE_NAME -c "
${l.replace(/"/g,'\\"').replace(/\\\\/g,"\\\\\\").replace(/\\x/g,"\\\\x")}
" -t -A > output.json;`:l},L=(n={})=>n.databaseEdition===a.MYSQL_5_7?`SET SESSION group_concat_max_len = 1000000; -- large enough value to handle your expected result size
SELECT CAST(CONCAT(
    '{"fk_info": [',
...
`,T=()=>`# Cloudflare D1 (via Wrangler CLI) Import Script
# ------------------------------------------------------
...
`,p=(n={})=>n.databaseEdition===a.SQLITE_CLOUDFLARE_D1?(n==null?void 0:n.databaseClient)===A.SQLITE_WRANGLER?T():d:C,O=`/* SQL Server 2017 and above edition (14.0, 15.0, 16.0, 17.0)*/
WITH fk_info AS (
    SELECT
        JSON_QUERY(
            N'[' + STRING_AGG(
...
`,R=`/* SQL Server 2016 and below edition (13.0, 12.0, 11.0..) */
WITH fk_info AS (
    SELECT  JSON_QUERY('[' +
        ISNULL(
            STUFF((
...
) AS metadata_json_to_import;`,u=(n={})=>n.databaseEdition===a.SQL_SERVER_2016_AND_BELOW?R:O,b=`WITH fk_info as (
  (SELECT (@fk_info:=NULL),
              (SELECT (0)
               FROM (SELECT kcu.table_schema,
                kcu.table_name,
...
`,g=`/* CockroachDB - PostgreSQL edition */
WITH fk_info AS (
...
cols AS (
    ...
    LEFT JOIN pg_catalog.pg_description dsc ON dsc.objoid = c.oid
                                        AND dsc.objsubid = cols.ordinal_position
    WHERE cols.table_schema NOT IN ('information_schema', 'pg_catalog')${I}
), indexes_metadata AS (
    ...
    SELECT array_to_string(array_agg(CONCAT('{',
        ...
        LEFT JOIN pg_catalog.pg_description dsc ON dsc.objoid = c.oid
                                                AND dsc.objsubid = 0
        WHERE tbls.TABLE_SCHEMA NOT IN ('information_schema', 'pg_catalog')${k}
), config AS (
    SELECT array_to_string(
                      array_agg(CONCAT('{"name":"', conf.name, '","value":"', replace(conf.setting, '"', E'"'), '"}')),
                      ',') AS config_metadata
    ...
    SELECT array_to_string(array_agg(CONCAT('{"schema":"', views.schemaname::TEXT,
                    ...
                    '"}')),
                      ',') AS views_metadata
    FROM pg_views views
    WHERE views.schemaname NOT IN ('information_schema', 'pg_catalog')${M}
)
SELECT CONCAT('{    "fk_info": [', COALESCE(fk_metadata, ''),
                    '], "pk_info": [', COALESCE(pk_metadata, ''),
                    '], "columns": [', COALESCE(cols_metadata, ''),
...
`,J={[t.GENERIC]:()=>"",[t.POSTGRESQL]:N,[t.MYSQL]:L,[t.SQLITE]:p,[t.SQL_SERVER]:u,[t.MARIADB]:()=>b,[t.CLICKHOUSE]:()=>f,[t.COCKROACHDB]:()=>g};export{J as importMetadataScripts};
